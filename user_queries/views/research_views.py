...
class ResearchEdit(APIView):
    ...
    def get_module_id(self, module_name, mongo):
        ...
        module = mongo.connect("modules").find_one({"name": module_name, "deleted_at":None}, {"_id": 1})
    ...
    def get_subgenders(self, mongo):
        ...
        subgenders = mongo.connect("subgenders").find({"deleted_at":None}, {"title": 1, "gender_id": 1})
        if not subgenders:
            return []
    ...
    def get(self, request, _id):
        ...
        involved_creation_research = cursor_change_json.get("involved_creation_info", [])
        """
        self.get_research_data(
        ...
        place_of_creation_research = cursor_change_json.get("place_of_creation_info", [])
        """
        self.get_research_data(
        ...
        period_research = self.get_research_data(
            mongo, cursor_change_json, "period_id"
        )
    ...
    def get_catalog_elements(self, mongo, catalog_id):
        ...
        if not catalog_id:
            return []
        elements = list(
            mongo.connect("catalog_elements").find({"catalog_id": ObjectId(catalog_id), "deleted_at":None},{"title": 1})
        )
